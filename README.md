...
- Both search by the game's name, which is **cleaned first**: the commercial wrapping Humble adds ("Buy …", "… on Humble Store", trademark symbols) is stripped so the search sees the name and nothing else. PCGamingWiki gets a bit more work: the edition suffix comes off, and on a DLC it is sent to the base game instead — the wiki documents DLC inside the game and has no page of its own for them. Humble names it only in the sentence it shows you ("requires the base game …"), which it draws with JavaScript, so if that notice arrives after the buttons the link is rewritten when it does. Being name searches they can still miss, so **each button says so in its tooltip** — the label carries the destination, the tooltip carries the uncertainty.
